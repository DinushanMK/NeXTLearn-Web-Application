...
import { Study_Material_Table, Chapter_Notes_Table, User_Table, Study_Type_Content_Table} from "@/configurations/dbschema";
import { GenerateStudyTypeContentAiModel } from "@/configurations/AiModel";
import { generateNotesAiModel } from "@/configurations/AiModel";
import { GenerateQuizAiModel } from "@/configurations/AiModel";
...
export const GenerateNotes=inngest.createFunction(
    ...
    async({event,step})=>{
        ...
        const notesResult=await step.run('Generate Chapter Notes',async()=>{
            ...
            //Chapters.forEach(async(chapter)=>{
                ...
                const aiResp=result.response.text();
                    console.log(PROMPT)
                await database.insert(Chapter_Notes_Table).values({
                    chapterId:index,
        ...
        const updateCourseStatusResult=await step.run('Update Course Status to Ready',async()=>{
            const result=await database.update(Study_Material_Table).set({
                status:'Ready'
...
// Used for generating flashcards and quiz
export const GenerateStudyTypeContent=inngest.createFunction(
    ...
    async({event,step})=>{
        ...
        const databaseResult=await step.run('Save Result to Database',async()=>{
            const result=await database.update(Study_Type_Content_Table)
            .set({
                content:AiResult,
                status:'Ready'
